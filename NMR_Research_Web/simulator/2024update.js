...
    
    // オフセット: -1.0〜+1.0 MHz
    sliders.offset = adj.makeSlider("Δ&omega;/2&pi;", -1.5, 1.5, params.offset, 0.02, 40);
...
    sliders.T2       = adj.makeSlider("T<sub>2</sub>", 10.0, 300.0, params.T2, 5.0, 240);
    sliders.tau      = adj.makeSlider("&tau;", 10.0, 80.0, params.tau, 1.0, 290);
...
function drawLabels() {
  let adj = new Adjuster(); 
  adj.drawValue(params.offset, 40, " MHz");
...
        
        // RF B_1 を固定
        // 例: 1H (γ=42.6 MHz/T) で 1.0 µs が90°になるように調整
        const GAMMA_REF = 42.6;   // [MHz/T]
        const T90_REF   = 1.0;    // [µs]
...
this.t_echo    = this.t1_center + 2 * this.tau;      // エコー中心（1st中心から 2tau 後）
    ...
    getSignal(t) {
        ...
        // --- detuning込みの有効角周波数（回転座標系） ---
// ω1 = 2π (γ/2π) B1 なので、gammaが [MHz/T] = cycles/(µs·T) の時は 2π を掛けて [rad/µs]
const omega1 = 2 * Math.PI * this.gamma * this.B_1;

// Δω = 2π * offset （offsetはΔω/2π = MHz = cycles/µs）
const deltaOmega = 2 * Math.PI * this.offset;

// 有効角周波数
const omegaEff = Math.sqrt(omega1 * omega1 + deltaOmega * deltaOmega);

// 回転軸（y軸周りで回す想定にしてるので n=(nx,0,nz)）
const nx = (omegaEff === 0) ? 0 : (omega1 / omegaEff);
const nz = (omegaEff === 0) ? 0 : (deltaOmega / omegaEff);

// --- 1stパルス後の横磁化比 A1（= M⊥/M0） ---
const th1 = omegaEff * this.t_pulse1;
const mx1 = nx * nz * (1 - Math.cos(th1));
const my1 = -nx * Math.sin(th1);
const A1  = Math.sqrt(mx1 * mx1 + my1 * my1);

// --- 2ndパルスの「反転効率」っぽい係数（半角） ---
const th2  = omegaEff * this.t_pulse2;
const mx2h = nx * nz * (1 - Math.cos(th2 / 2));
const my2h = -nx * Math.sin(th2 / 2);
const A2h  = Math.sqrt(mx2h * mx2h + my2h * my2h);

// --- 時間領域ごとの振幅係数 ---
let currentPulseFactor;
if (tRel < this.tau) {
  currentPulseFactor = A1;              // FID領域
} else {
  currentPulseFactor = A1 * (A2h * A2h); // Echo領域（簡易）
}
...
// --- 0（ゼロ線=this.Y_0）基準の水平グリッド ---
const gridPix = 30;         // 1本ごとの間隔[px]（好みで 20〜40）
stroke(60);                 // 薄いグレー
strokeWeight(1);

// 上側（正）
for (let y = this.Y_0 - gridPix; y > 10; y -= gridPix) {
  line(this.X_0, y, this.GraphWidth, y);
}

// 下側（負）
for (let y = this.Y_0 + gridPix; y < this.canvasHeight - 10; y += gridPix) {
  line(this.X_0, y, this.GraphWidth, y);
}
